...
(or `bacpypes3`/`BAC0`). This repository's own verification session, against a
...
[v1.0.0](https://github.com/chipkin/BACnetProfileExample-B-AEC-CPP/releases/tag/v1.0.0)
(`metrics-windows.json` / `metrics-linux.json`):

<!-- METRICS -->
| Platform | Binary | Size | Start-up to `ready` | Stack commit |
|---|---|---|---|---|
| Windows x64 (windows-2022) | `BACnetExampleBAEC.exe` | 3,439,104 bytes (~3.3 MiB) | 242 ms | `abd4cee1` |
| Linux x64 (ubuntu-latest) | `BACnetExampleBAEC` | 69,928 bytes (~68 KiB) | 114 ms | `abd4cee1` |

v1.0.0 was published from a prebuilt STATIC-library build; the next release
refreshes these numbers from the SOURCE-mode build documented above.
